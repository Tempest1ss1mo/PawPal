// PawPal Web Application - Main JavaScript (Static Deployment Version)
// Uses CONFIG from config.js for API URL and Google Client ID
...
async function handleGoogleCredentialResponse(response) {
    ...
    try {
        const result = await fetch(CONFIG.API_BASE_URL + '/api/auth/google', {
            method: 'POST',
            headers: {
        ...
        if (data.success) {
            if (data.isNewUser) {
                ...
                // Store user in localStorage for static deployment
                localStorage.setItem('pawpal_user', JSON.stringify(currentUser));
...
async function checkLoginStatus() {
    // First check localStorage for static deployment
    const storedUser = localStorage.getItem('pawpal_user');
    if (storedUser) {
        try {
...
async function handleGoogleComplete(e) {
    ...
    try {
        const response = await fetch(CONFIG.API_BASE_URL + '/api/auth/google/complete-registration', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function handleCreateWalk(e) {
    ...
    try {
        const response = await fetch(CONFIG.API_BASE_URL + '/api/walks', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function loadMyWalks(status) {
    try {
        const response = await fetch(CONFIG.API_BASE_URL + `/api/walks?status=${status}`, {
            credentials: 'include'
        });
        const result = await response.json();
...
async function cancelWalk(walkId) {
    ...
    try {
        const response = await fetch(CONFIG.API_BASE_URL + `/api/walks/${walkId}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function loadAvailableWalks(city = '') {
    try {
        let url = CONFIG.API_BASE_URL + '/api/walks?status=requested';
        if (city) {
...
async function acceptWalk(walkId) {
    ...
    try {
        const response = await fetch(CONFIG.API_BASE_URL + '/api/assignments', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function loadMyJobs(status) {
    try {
        const response = await fetch(CONFIG.API_BASE_URL + `/api/assignments?status=${status}`, {
            credentials: 'include'
        });
        const result = await response.json();
...
async function startWalk(assignmentId) {
    try {
        const response = await fetch(CONFIG.API_BASE_URL + `/api/assignments/${assignmentId}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
...
console.log('PawPal Web Application Initialized (Static Deployment)');
